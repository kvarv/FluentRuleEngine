...
namespace Tests
{
	public class Tests
	{
		...
		{
			var task = new Task();
			var engine = new TaskRuleEngine();
			engine.SetUp(task);
			Assert.Equal(1, engine.RuleSets.Count);
		...
		{
			var task = new Task {RemainingHours = 0, Estimate = 10};
			var engine = new TaskRuleEngine();
			engine.SetUp(task);
			engine.Execute();
		...
		{
			var task = new Task {RemainingHours = 0, Estimate = 5};
			var engine = new TaskRuleEngine();
			engine.SetUp(task);
			engine.Execute();
